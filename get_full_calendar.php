...
while ($currentDay <= $lastDay) {
    ...
    $getReserveSql = "SELECT * FROM booked_list WHERE service_id='$serviceId' AND start_date ='$real_date'";
    $result = $conn->query($getReserveSql);
    $reserveRow = $result->fetch_assoc();

    $reserveCount = $result->num_rows; //0이면 해당날짜는 예약이 없는것

    
    $clickDate = strtotime($real_date); //오늘 이상의 값만 달력에 표기하기 위한 변수
    if($clickDate>=$today){
        if($reserveCount==0){
            echo "<div class='reserve_data'><a href='./reservation.php?date=$real_date&service_id=$serviceId'><span class='possible'>예</span><span class='txt'>$serviceName</span></div></a></td>";
        }else{
            if($reserveRow['booked_ok']==1){
                echo "<div class='reserve_data'><a><span class='booked'>완</span><span class='txt'>$serviceName</span></a></div></td>";
            }else if($reserveRow['booked_ok']==0){
                echo "<div class='reserve_data'><a><span class='booking'>대</span><span class='txt'>$serviceName</span></div></a></td>";
            }
        }
    }    
