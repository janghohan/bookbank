...
while ($currentDay <= $lastDay) {
    ...
    $getReserveSql = "SELECT * FROM booked_list WHERE service_id='$service_id' AND start_date ='$real_date'";
    $result = $conn->query($getReserveSql);
    $reserveRow = $result->fetch_assoc();

    $clickDate = strtotime($real_date);
    if($clickDate>=$today){
        if($reserveRow['booked_ok']==2){
            echo "<div class='reserve_data'><a><span class='booked'>완</span><span class='txt'>$serviceName</span></a></div></td>";
        }else if($reserveRow['booked_ok']==1){
            echo "<div class='reserve_data'><a><span class='booking'>대</span><span class='txt'>$serviceName</span></div></a></td>";
        }else{
            echo "<div class='reserve_data'><a href='./reservation.php?date=$real_date&service_id=$service_id'><span class='possible'>예</span><span class='txt'>$serviceName</span></div></a></td>";
        }
    }
